...
void updateCursor(int newCol = -1, int newRow = -1)
{
  u8g2.setCursor((newCol == -1 ? col : newCol) * (u8g2.getMaxCharWidth()), (1 + (newRow == -1 ? row : newRow)) * (u8g2.getMaxCharHeight()));
  //u8g2.sendBuffer();
}
...
void LcdScrollUp(void)
{
  ...
  for ( i = 0; i < (lcd_nrows - 1); i++ )
  {
    ...
    //u8g2.setCursor(0*u8g2.getMaxCharHeight(), i*u8g2.getMaxCharWidth());
    updateCursor(0, i);
    for ( j = 0; j < lcd_ncols && lcdRows[i][j] != NUL; j++ )
    {
      u8g2.print(lcdRows[i][j]);
    }
  }
      ...
      switch (ctrlseq_p[0])
      {
        case 1:
          switch (ctrlseq_p[1])
          {
            ...
            case 9:
              Serial.print("[");
              Serial.print(lcd_nrows);
              Serial.print(";");
              Serial.print(lcd_ncols);
              Serial.print("t");
              break;
          }
          break;
